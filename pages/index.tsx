...
import { CommitCard } from '../lib/component/CommitCard';
import { User } from './api/auth/checklogin';
...
const Index = ({ authUrl }: InferGetStaticPropsType<typeof getStaticProps>) => {
    ...
    const checkLogin = useCallback(async () => {
        const userResponse = await axios.get<User>('/api/auth/checklogin');
                ...
                <div className='bg-sky-900/20 h-full'>
                    <header className='fixed w-full h-16 backdrop-blur px-4'>
                        <div className='max-w-5xl mx-auto flex  h-full'>
                            <div className='text-4xl font-bold tracking-[-0.07em] h-auto my-auto'>
                                commit-timeline
                            </div>
                            ...
                            {user && (
                                <div
                                    ...
                                    style={{
                                        ...
                                        left: (accountButtonRef.current?.getBoundingClientRect().right || 0) - (accountMenuRef.current?.clientWidth || 0)
                                    }}
                                    ref={accountMenuRef}
                                >
                            ...
                            <div className='grid grid-cols-8'>
                                <div className='col-span-2'>
                                    <div className='p-4 sticky top-16'>
                                        <div>
                                            <div className='text-lg mb-4 h-8'>
                                                SINCE
                                            </div>
                                            <div className='text-lg'>
                                                <select className='py-2 px-4 font-bold bg-sky-700/20 outline-none border-none focus:outline-none' value={JSON.stringify(since)}
                                                    onChange={(e) => {
                                        ...
                                        <div className='mt-6'>
                                            ...
                                            <div>
                                                {groupBys.map((targetGroupby) =>
                                                    <div
                                                        className={`text-lg p-3 cursor-pointer hover:bg-sky-700/20 duration-200 ${groupby === targetGroupby ? 'font-bold bg-sky-700/20' : ''}`}
                                                        onClick={() => {
                                    ...
                                    {user ? (
                                        <div>
                                            {loading ? (
                                                ...
                                                <div>
                                                    {groupByCommits && Array.from(groupByCommits.keys()).map((key) =>
                                        ...
                                        <div className='flex p-4'>
                                            <button
                                                ...
                                                <div className='ml-2 h-auto my-auto'>
                                                    Sign in with GitHub
                                                </div>
                                            </button>
                                        </div>
                    ...
                    <footer className='h-12 text-sm text-gray-300 text-center'>VALUES, Inc. All rights Reserved</footer>
                </div>
            </div>
        </div >
    )
