...
const formatCommitsFromRepo = (repos: Repository[]) => {
    ...
    repos.forEach((repository) => {
        repository.refs.nodes.forEach((branch) => {
            ...
            branch.target.history.nodes.forEach(({ url, message, committedDate, oid, changedFiles, additions, deletions }) => {
                commits.push({
                    sha: oid,
                    url,
                    message,
...
const fetchEachRepositoryCommits = async (repository: ContributedRepository, accessToken: string, email: string, since: string) => {
    const commits: Commit[] = [];
    let hasNext = true;
...
export const commitListRoute = async (
    req: NextApiRequest,
    res: NextApiResponse<{}>
) => {
