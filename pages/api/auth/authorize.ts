...
const authAuthorizeRoute = async (
    req: NextApiRequest,
    res: NextApiResponse<{}>
) => {
    try {
        ...
        const tokenResponse = await axios.post<{}, AxiosResponse<{ access_token: string, scope: string, token_type: string }>>(`https://github.com/login/oauth/access_token`, params, {
            headers: {
        ...
        const userResponse = await axios.get<{ avatar_url: string, login: string, html_url: string }>('https://api.github.com/user', {
            headers: {
